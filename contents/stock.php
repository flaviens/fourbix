...
<div class="item">
	...
	<div class="row">
		<div class="col-md-4 col-md-offset-2">
			<p><b>Binet : </b><?php echo htmlspecialchars($stock->binet) ?></p>
			<p><b>Marque : </b><?php echo htmlspecialchars($item->marque) ?></p>
			<p><b>Type : </b><?php echo htmlspecialchars($item->type) ?></p>
			<p><b>Déscription : </b><br/><?php echo htmlspecialchars($stock->description) ?></p>
			<p><b>Quantité disponible : </b><?php echo htmlspecialchars($stock->quantite) ?></p>
			<p><b>Caution : </b><?php echo htmlspecialchars($stock->caution) ?> &euro;</p>
		</div>
		<div class="col-md-4">
			<img src=<?php echo "'images/items/{$stock->image}'"; ?> class="image-item-stock">
		</div>
	</div>
</div>
...
<?php
		$valid_demande = false;
		if($stock->quantite > 0){
			if(isset($_POST['quantite']) and $_POST['quantite'] >= 1 and $_POST['quantite'] <= htmlspecialchars($stock->quantite)){
				...
				if ($valid_demande){
					$sth = $dbh->prepare("INSERT INTO demandes (utilisateur, stock_id, binet, commentaire, debut, fin, binet_emprunteur, quantite) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
					$sth->execute(array($_SESSION["login"], $stock->id, $stock->binet, $commentaire, $debut, $fin, $binet_emprunteur, $quantite));
					//$sth->execute(array($_SESSION["login"], $stock->binet, $commentaire, $_POST['date-debut'], $_POST['date-fin'], $_POST['binet'], $quantite));
				}
				else
